...
def load_aws_credentials(config_path:str) -> None:
    """
    ...
    try:
        ...
        aws_access_key_id = config["IAM"]["AWS_ACCESS_KEY_ID"]
        aws_secret_access_key = config["IAM"]["AWS_SECRET_ACCESS_KEY"]
...
def create_spark_session() -> Union[SparkSession, None]:
    """
...
def process_song_data(spark: SparkSession, input_data:str, output_data:str) -> None:
    ...
    try:
        ...
        print(f"Error processing song data: {str(e)}")
...
def main():
    config_path = "dl.cfg"
    load_aws_credentials(config_path)
